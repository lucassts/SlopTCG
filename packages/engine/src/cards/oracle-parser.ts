...
function parseSearch(clause: string): EffectStep[] | null {
  // Leva 5: o substantivo passa pela gramática ("Rebel permanent card with mana value 3 or less", "instant or sorcery card").
  const m = clause.match(
    /^search your library for (?:up to (\w+) )?(?:(a|an|one|two|three|four) )?(.+?)(?:, reveal (?:it|them|that card|those cards))?,? (?:and )?put (?:it|them|that card|those cards|those lands) (into your hand|onto the battlefield tapped|onto the battlefield|on top of your library|into your graveyard)(?:, then shuffle| and shuffle|\. then shuffle)?$/i,
  );
  if (!m) return null;
  const count = m[1] ? num(m[1]) : m[2] && !/^an?$/i.test(m[2]) ? num(m[2]) : 1;
  if (count === null) return null;
  const info = parseNounG(m[3]);
  if (!info || info.player || info.zone || info.spell || info.any) return null;
  const filter: FilterSpec | undefined = Object.keys(info.filter).length > 0 ? info.filter : undefined;
  const dest = m[4].toLowerCase();
  const to = dest === 'into your hand' ? 'hand' : dest === 'on top of your library' ? 'libraryTop' : dest === 'into your graveyard' ? 'graveyard' : 'battlefield';
...
function parseEffectText(
  text: string,
  gctx: { pronoun: SubjectRef; pronounPlayer: WhoSel; /** Targets already introduced by earlier lines of the same spell ("that creature"). */ priorSpecs?: TargetSpec[] } = { pronoun: 'self', pronounPlayer: 'controller' },
): { steps: EffectStep[]; spec?: TargetSpec; specs?: TargetSpec[]; selfExile?: boolean; kickerSteps?: EffectStep[] } | null {
  let m: RegExpMatchArray | null;
  ...
  if ((m = text.match(/^Look at the top (\w+) cards? of your library\. Put (\w+) of them into your hand and the rest on the bottom of your library in (?:any|a random) order\. If (.+?), instead put (\w+) of them into your hand and the rest on the bottom of your library in (?:any|a random) order\.\s*(.*)$/i))) {
    const n = num(m[1]); const a = num(m[2]); const b = num(m[4]); const cond = parseCondG(m[3]);
    if (n === null || a === null || b === null || !cond) return null;
    steps.push({ op: 'if', cond, then: [{ op: 'digTop', count: n, pick: b, rest: 'bottom' }], else: [{ op: 'digTop', count: n, pick: a, rest: 'bottom' }] });
    text = m[5];
    if (!text.trim()) return { steps };
  }
  // "Look at the top N cards of your library. Put one of them into your hand and the rest…" — as duas frases formam uma só instrução (gramática).
  text = text.replace(/^(Look at the top \w+ cards? of your library)\. ((?:You may )?[Pp]ut (?:one of them|up to \w+ of them|\w+ of them|(?:a|an) [\w\s-]+? card from among them) [^.]*\.)/i, '$1 $2');
  // "Reveal the top card of your library. If it's a X card, put it into your hand. Otherwise, put it on the bottom/into your graveyard."
  if ((m = text.match(/^Reveal the top card of your library\. If it's (?:a|an) ([\w\s-]+?) card, put it into your hand\. Otherwise, put it (on the bottom of your library|into your graveyard)\.?\s*(.*)$/i))) {
    const info = parseNounG(m[1]);
    if (!info || info.player) return null;
    steps.push({ op: 'digTop', count: 1, pick: 1, filter: info.filter, rest: /graveyard/i.test(m[2]) ? 'graveyard' : 'bottom' });
    text = m[3];
    if (!text.trim()) return { steps };
  }
  ...
  // "Look at the top N…. You may reveal a X card from among them and put it into your hand. Put the rest…" (Once Upon a Time).
  text = text.replace(/^(Look at the top \w+ cards? of your library)\. (You may reveal (?:a|an) [\w\s,-]+? card from among them and put it into your hand)\. Put the rest ([^.]*\.)/i, '$1 $2, then put the rest $3');
  // Malevolent Rumble: "Reveal the top N cards of your library. You may put a X card from among them into your hand. Put the rest into your graveyard."
  if ((m = text.match(/^Reveal the top (\w+) cards? of your library\. You may put (?:a|an) ([\w\s-]+?) card from among them into your hand\. Put the rest (into your graveyard|on the bottom of your library in a random order)\.\s*(.*)$/i))) {
    const n = num(m[1]); const info = parseNounG(m[2]);
    if (n === null || !info || info.player) return null;
    steps.push({ op: 'digTop', count: n, pick: 1, filter: info.filter, rest: /graveyard/i.test(m[3]) ? 'graveyard' : 'bottom' });
    text = m[4];
    if (!text.trim()) return { steps };
  }
  ...
  // Goblin Welder.
  if (/^Choose target artifact a player controls and target artifact card in that player's graveyard\. If both targets are still legal as this ability resolves, that player simultaneously sacrifices the artifact and returns the artifact card to the battlefield\.$/i.test(text)) {
    return { steps: [{ op: 'welderSwap' }], specs: [{ what: 'artifact' }, { what: 'artifact', zone: 'graveyard' }] };
  }
  // Fury: "it deals N damage divided as you choose among any number of target creatures or planeswalkers".
  if ((m = text.match(/^(?:it|~) deals (\d+) damage divided as you choose among any number of target (.+?)\.?$/i))) {
    const info = parseNounG(m[2]) ?? parseNounG(m[2].replace(/s$/i, ''));
    const spec = info ? filterToTargetSpec(info) : null;
    if (!spec) return null;
    return { steps: [{ op: 'divideDamage', amount: parseInt(m[1], 10) }], specs: Array.from({ length: 6 }, () => ({ ...spec, optional: true })) };
  }
  // Portent of Calamity.
  if (/^Reveal the top X cards of your library\. For each card type, you may exile a card of that type from among them\. Put the rest into your graveyard\. You may cast a spell from among the exiled cards without paying its mana cost if you exiled four or more cards this way\. Then put the rest of the exiled cards into your hand\.$/i.test(text)) {
    return { steps: [{ op: 'portentReveal', count: 'X' }, { op: 'portentCast' }] };
  }
  // Price of Progress.
  if ((m = text.match(/^~ deals damage to each player equal to (twice )?the number of (.+?) that player controls\.$/i))) {
    const info = parseNounG(m[2]);
    if (!info || info.player) return null;
    return { steps: [{ op: 'damageEachPlayerPer', filter: info.filter, times: m[1] ? 2 : 1 }] };
  }
  // Triumph of Saint Katherine.
  if ((m = text.match(/^exile it and the top (\w+) cards of your library in a face-down pile\. If you do, shuffle that pile and put it back on top of your library\.$/i))) {
    const n = num(m[1]);
    return n === null ? null : { steps: [{ op: 'exileSelfAndTopShuffleBack', count: n }] };
  }
  // Peer into the Abyss.
  ...
  for (const rawSentence of sentences) {
    ...
    if (/^exile this card$/i.test(sentence)) { steps.push({ op: 'exile', what: 'self' }); continue; }
    ...
    // "Creatures without flying can't block this turn." (Sundering Eruption)
    if ((m = sentence.match(/^Creatures without (\w+) can't block this turn$/i)) && KEYWORDS[m[1].toLowerCase()]) {
      steps.push({ op: 'forEach', filter: { what: 'creature', withoutKeyword: KEYWORDS[m[1].toLowerCase()] }, effect: [{ op: 'pump', what: 'iter', power: 0, toughness: 0, keywords: ['cantBlock'] }] });
      continue;
    }
    // Poxwalkers / Bloodghast: "(you may) return this card from your graveyard to the battlefield (tapped)".
    if ((m = sentence.match(/^(you may )?return (?:~|this card) from your graveyard to the battlefield( tapped)?$/i))) {
      const ret: EffectStep = { op: 'returnToBattlefield', what: 'self', tapped: !!m[2] || undefined };
      steps.push(m[1] ? { op: 'mayDo', prompt: 'devolver esta carta do cemitério ao campo de batalha?', effect: [ret] } : ret);
      continue;
    }
    ...
    if ((m = sentence.match(/^Gain control of target (noncreature )?spell$/i))) {
      ...
      spec = { what: 'spell', ...(m[1] ? { noncreature: true } : {}) } as TargetSpec;
      steps.push({ op: 'gainControlSpell', what: 'target:0' });
      continue;
    }
    if (/^You may choose new targets for it$/i.test(sentence)) continue;
    // "It explores." depois de uma frase com alvo: o alvo explora (Seasoned Dungeoneer).
    if (/^It explores$/i.test(sentence) && (specs?.length || spec)) { steps.push({ op: 'explore', what: `target:${(specs ?? [spec!]).length - 1}` }); continue; }
    ...
    if (!parsed) {
      ...
      if (parts.length >= 2) {
        ...
        for (const p of parts) {
          ...
          if (pp.spec) {
            if (partSpec || spec) { ok = false; break; }
            partSpec = pp.spec;
          }
          acc.push(...pp.steps);
        }
        if (ok) parsed = { steps: acc, spec: partSpec };
      }
    }
    if (!parsed) {
      // Leva 4: gramática composicional (sujeito × verbo × quantidade × condição × duração; vários alvos).
      const prior = specs ?? (spec ? [spec] : [...(gctx.priorSpecs ?? [])]);
      ...
      if (!g) {
        ...
        if (parts.length >= 2) {
          ...
          if (ok) g = { steps: acc, specs: accSpecs };
        }
      }
      if (!g) return null;
      if (g.specs.length > 0) { specs = [...prior, ...g.specs]; spec = undefined; }
      parsed = { steps: g.steps };
    }
    if (parsed.spec) {
  ...
  // Specs inherited from earlier lines are not new targets of this text.
  const inherited = gctx.priorSpecs?.length ?? 0;
  if (inherited > 0 && specs && specs.length >= inherited && specs.slice(0, inherited).every((x, i) => x === gctx.priorSpecs![i])) {
    specs = specs.length > inherited ? specs.slice(inherited) : undefined;
  }
...
interface ParseState {
  ...
  /** Leva 4: substituição, custos, estáticas diversas. */
  flags5: Partial<Pick<CardDefinition, 'exileInsteadOfDying' | 'exileDyingCreatures' | 'preventAllDamageToSelf' | 'lifeGainModifier' | 'tokenDoubling' | 'attackRequiresDefenderSubtype' | 'mustBeBlocked' | 'skipDraw' | 'cantBeBlockedBy' | 'evasionPowerLessThanSelf' | 'prototype' | 'canExert' | 'entersWithCountersIf' | 'copyOnEnter'>>;
  /** Leva 5: gramática 2 — estáticas, substituições, gatilho de reciclar. */
...
/** Mana value of a cost string ("{1}{R}" → 2). */
function manaValueOfCost(cost: string): number {
  let mv = 0;
  for (const mm of cost.matchAll(/\{([^}]+)\}/g)) mv += /^\d+$/.test(mm[1]) ? parseInt(mm[1], 10) : mm[1] === 'X' ? 0 : 1;
  return mv;
}
...
function parseActivationCost(text: string): Extract<AbilityDef, { kind: 'activated' }>['cost'] | null {
  ...
  for (const tokRaw of text.split(/,\s*/)) {
    ...
    if ((m = tok.match(/^Sacrifice (?:a|an|another) (.+)$/i))) {
      const info = parseNounG(m[1]);
      if (!info || info.player || info.zone || info.spell || info.any) return null;
    ...
    if ((m = tok.match(/^Remove (a|an|\w+) ([\w+/-]+) counters? from ~$/i))) {
      const n = /^an?$/i.test(m[1]) ? 1 : num(m[1]);
      if (n === null) return null;
      cost.removeCounters = { counter: m[2], count: n }; any = true; continue;
    }
    if ((m = tok.match(/^Exile (a|an|\w+) (.+?) cards? from your graveyard$/i))) {
      const n = /^an?$/i.test(m[1]) ? 1 : num(m[1]);
      const info = parseNounG(m[2]);
      if (n === null || !info || info.player || info.zone) return null;
      cost.exileFromGraveyard = { filter: info.filter, count: n }; any = true; continue;
    }
    if ((m = tok.match(/^Exile (a|an|\w+) cards? from your graveyard$/i))) {
      const n = /^an?$/i.test(m[1]) ? 1 : num(m[1]);
      if (n === null) return null;
      cost.exileFromGraveyard = { filter: {}, count: n }; any = true; continue;
    }
    if (/^Tap an untapped creature you control$/i.test(tok)) { cost.tapCreature = true; any = true; continue; }
    if (/^Return a land you control to its owner's hand$/i.test(tok)) { cost.returnLand = true; any = true; continue; }
    if (/^Exile ~$/i.test(tok)) { cost.exileSelfFromBattlefield = true; any = true; continue; }
    if (/^Discard (?:~|this card)$/i.test(tok)) { cost.discardSelf = true; any = true; continue; }
    if (/^Discard your hand$/i.test(tok)) { cost.discardHand = true; any = true; continue; }
    if (/^Exile (?:~|this card) from your hand$/i.test(tok)) { cost.exileSelfFromHand = true; any = true; continue; }
    return null; // marcadores em quantidade livre etc. fora da DSL
  }
...
function parseTriggerHeader(head: string): { trigger: TriggerSpec; extraSelf?: TriggerSpec; oncePerTurn?: boolean; condition?: Cond } | null {
  ...
  if (/^When (?:~|this card) is put into your graveyard from your library$/i.test(head)) return { trigger: { on: 'toGraveyardFromAnywhere', self: true, fromZone: 'library' } };
  if (/^When ~ is put into your graveyard from the battlefield$/i.test(head)) return { trigger: { on: 'dies', self: true } };
  ...
  // ---- Leva 4
  if (/^At the beginning of combat on your turn$/i.test(head)) return { trigger: { on: 'beginCombat', whose: 'controller' } };
  if (/^At the beginning of (?:each combat|combat on each player's turn)$/i.test(head)) return { trigger: { on: 'beginCombat', whose: 'each' } };
  if (/^At the beginning of your (?:first main phase|precombat main phase)$/i.test(head)) return { trigger: { on: 'main1', whose: 'controller' } };
  if (/^At the beginning of your (?:second main phase|postcombat main phase)$/i.test(head)) return { trigger: { on: 'main2', whose: 'controller' } };
  if (/^At the beginning of each opponent's (upkeep|end step)$/i.test(head)) return { trigger: (/upkeep/i.test(head) ? { on: 'upkeep', whose: 'opponent' } : { on: 'endStep', whose: 'opponent' }) as unknown as TriggerSpec };
  if (/^When ~ is turned face up$/i.test(head)) return { trigger: { on: 'turnedFaceUp', self: true } };
  if (/^(?:Enrage — )?Whenever ~ is dealt damage$/i.test(head)) return { trigger: { on: 'dealtDamage', self: true } };
  if (/^Whenever ~ deals damage(?: to a creature| to a player| to an opponent)?$/i.test(head)) return { trigger: { on: 'dealsDamage', self: true } };
  if (/^Whenever ~ deals combat damage to a creature$/i.test(head)) return { trigger: { on: 'combatDamageToCreature', self: true } };
  ...
  if (/^Whenever (?:enchanted|equipped) creature is dealt damage$/i.test(head)) return { trigger: { on: 'hostDealtDamage' } };
  if (/^Whenever a player casts a spell$/i.test(head)) return { trigger: { on: 'anyCastsSpell' } };
  if (/^Whenever (?:a|one or more) creatures? you control deals? combat damage to a player$/i.test(head)) return { trigger: { on: 'yourCreatureCombatDamageToPlayer' } };
  if (/^Whenever you exert ~$/i.test(head)) return { trigger: { on: 'youExertThis' } };
  if ((m = head.match(/^(?:Whenever|When) you draw your (second|third) card (?:each turn|in a turn)$/i))) return { trigger: { on: 'youDrawCardNth', nth: m[1].toLowerCase() === 'second' ? 2 : 3 } };
  ...
  if ((m = head.match(/^Whenever you cast (?:a|an) ([A-Z][a-z]+(?: or [A-Z][a-z]+)*|artifact|enchantment|instant|sorcery|noncreature) spell$/))) {
    ...
    if (/^noncreature$/i.test(w)) return { trigger: { on: 'youCastSpell', noncreatureOnly: true } };
    const subs = w.split(' or ');
    return { trigger: { on: 'youCastSpellOf', filter: subs.length > 1 ? { subtypeAnyOf: subs } : { subtype: subs[0] } } };
  }
  if (/^(?:Heroic — )?Whenever you cast a spell that targets ~$/i.test(head)) return { trigger: { on: 'youCastSpellTargetingThis' } };
  if ((m = head.match(/^Whenever you sacrifice (?:a|an|another) (.+)$/i))) {
    const info = parseNounG(m[1]);
    return info && !info.player ? { trigger: { on: 'youSacrifice', filter: Object.keys(info.filter).length ? info.filter : undefined } } : null;
  }
  if (/^Whenever (?:a player|an opponent) discards a card$/i.test(head)) return { trigger: { on: 'anyPlayerDiscards' } };
  if (/^Whenever you discard a card$/i.test(head)) return { trigger: { on: 'youDiscard' } };
  if ((m = head.match(/^Whenever ~ or another ([A-Z][a-z]+) you control enters$/))) return { trigger: { on: 'etb', what: { what: 'creature', subtype: m[1], controlledBy: 'you', other: true } }, extraSelf: { on: 'etb', self: true } };
  if ((m = head.match(/^Whenever (?:another|a|an) (.+?) (enters|dies)(?: the battlefield)?(?: under your control)?$/i))) {
    const info = parseNounG(m[1]);
    if (info && !info.player && !info.zone) {
      const on = m[2].toLowerCase() === 'enters' ? 'etb' : 'dies';
      const filter: FilterSpec = { ...info.filter, other: true, controlledBy: /under your control$/i.test(head) ? 'you' : info.filter.controlledBy ?? 'any' };
      const another = /^Whenever another/i.test(head);
      const self = on === 'etb' ? { on: 'etb' as const, self: true as const } : { on: 'dies' as const, self: true as const };
      return { trigger: on === 'etb' ? { on: 'etb', what: filter } : { on: 'dies', what: filter }, extraSelf: another || filter.controlledBy === 'opponent' ? undefined : self };
    }
  }
  if (/^(?:Valiant — )?Whenever ~ becomes the target of a spell or ability you control for the first time each turn$/i.test(head)) return { trigger: { on: 'becomesTargeted', self: true }, oncePerTurn: true };
  if ((m = head.match(/^Whenever ~ blocks (?:a|an) (.+)$/i))) {
    const info = parseNounG(m[1]);
    if (!info || info.player) return null;
    return { trigger: { on: 'blocks', self: true }, condition: { kind: 'subjectIs', ref: 'triggering', filter: info.filter } };
  }
  ...
  if (/^Whenever you cast a spell from anywhere other than your hand$/i.test(head)) return { trigger: { on: 'youCastSpell', notFromHand: true } };
  if (/^Whenever a nontoken creature is put into your graveyard from the battlefield$/i.test(head)) return { trigger: { on: 'dies', what: { what: 'creature', nontoken: true, controlledBy: 'you' } } };
  if (/^When(?:ever)? a creature is put into an opponent's graveyard from the battlefield$/i.test(head)) return { trigger: { on: 'dies', what: { what: 'creature', controlledBy: 'opponent' } } };
  ...
  if ((m = head.match(/^Whenever ~ or another (.+?) you control enters$/i))) {
    ...
    return { trigger: { on: 'etb', what: { ...info.filter, controlledBy: 'you', other: true } }, extraSelf: { on: 'etb', self: true } };
  }
  if (/^Whenever ~ attacks alone$/i.test(head)) return { trigger: { on: 'attacks', self: true }, condition: { kind: 'attackedAlone' } };
  if ((m = head.match(/^Whenever ~ attacks with (\w+) or more other creatures$/i)) || (m = head.match(/^Whenever ~ and at least (\w+) other creatures attack$/i))) {
    const n = num(m[1]);
    return n === null ? null : { trigger: { on: 'attacks', self: true }, condition: { kind: 'attackersAtLeast', count: n + 1 } };
  }
  if (/^At the beginning of each upkeep$/i.test(head)) return { trigger: { on: 'upkeep', whose: 'each' } };
  if (/^Whenever you cast a multicolored spell$/i.test(head)) return { trigger: { on: 'youCastSpellOf', filter: { multicolored: true } } };
...
function parseLine(rawLine: string, st: ParseState, isSpell: boolean, subtypes: string[]): boolean {
  ...
  if (isSpell && (m = line.match(/^Choose (one|two|three|one or both|one or more|up to one|up to two|up to three|any number) —$/i))) {
    ...
    st.spellModeChoice =
      ...
      : k === 'two' ? { min: 2, max: 2 }
      : k === 'three' ? { min: 3, max: 3 }
      : k === 'one or both' ? { min: 1, max: 2 }
      : k === 'one or more' ? { min: 1, max: 99 }
      : k === 'up to one' ? { min: 0, max: 1 }
      : k === 'up to two' ? { min: 0, max: 2 }
      : k === 'up to three' ? { min: 0, max: 3 }
      : { min: 0, max: 99 };
    return true;
  }
  ...
  // Spree: "+ {1} — Efeito." — cada modo escolhido soma o próprio custo.
  if (isSpell && /^Spree$/i.test(line)) { st.modalOpen = true; st.spree = true; st.spellModeChoice = { min: 1, max: 99 }; return true; }
  if (st.spree && (m = line.match(/^\+ ((?:\{[^}]+\})+) — (.+)$/))) {
    const parsed = parseEffectText(m[2].trim().replace(/\.?$/, '.'));
    if (!parsed) return false;
    st.spellModes.push({ label: `${m[1]} — ${m[2].trim()}`, targets: specsOf(parsed), effect: parsed.steps, cost: m[1] });
    return true;
  }
  // Gatilho modal de permanente: "When ~ enters/dies / Whenever ~ attacks, choose one —" + bullets.
  if ((m = line.match(/^((?:Landfall — )?(?:When|Whenever|At the beginning of)[^,]+), choose (?:one|up to one) —$/i))) {
    const header = parseTriggerHeader(m[1].trim());
    if (!header) return false;
    st.modalTrigger = header.trigger;
    st.modalTriggerModes = [];
    return true;
  }
  if (st.modalTrigger && line.startsWith('• ')) {
    const parsed = parseEffectText(line.slice(2).trim().replace(/\.?$/, '.'), { pronoun: 'what' in st.modalTrigger ? 'triggering' : 'self', pronounPlayer: 'controller' });
    if (!parsed) return false;
  ...
  if ((m = line.match(/^(?:As long as (.+?), (.+)|(.+) as long as (.+))\.$/i)) && parseCondG(m[1] ?? m[4])) {
    ...
    const cond: Cond = parseCondG(m[1] ?? m[4])!;
    let sm: RegExpMatchArray | null;
    // "~ gets +N/+N (and has X)" / "~ has X" sobre a própria carta.
    if ((sm = inner.match(/^(?:~|it) gets ([+-]\d+)\/([+-]\d+)(?:,? (?:and )?has (\w[\w\s,]*?))?(?:,? and (?:attacks|must attack) each combat if able)?$/i)) || (sm = inner.match(/^(?:~|it) has (\w[\w\s,]*?)$/i))) {
      const mustAttack = /attacks? each combat if able$/i.test(inner);
      ...
      st.abilities.push({ kind: 'static', selfOnly: true, filter: {}, condition: cond, power: isPT ? parseInt(sm[1], 10) : undefined, toughness: isPT ? parseInt(sm[2], 10) : undefined, keywords: [...(kws ?? []), ...(mustAttack ? ['mustAttack' as const] : [])].length > 0 ? [...(kws ?? []), ...(mustAttack ? ['mustAttack' as const] : [])] : undefined, text: line });
      return true;
    }
    const before = st.abilities.length;
    ...
    if (added.length === 0 || added.some((a) => a.kind !== 'static')) { st.abilities.length = before; return false; }
    for (const a of added) if (a.kind === 'static') a.condition = cond;
    return true;
  }
  // ---- Leva 4: custos de mágicas, substituição, estáticas diversas
  if ((m = line.match(/^(?:(Colorless) )?(?:(Creature|Noncreature|Instant and sorcery|Artifact|Enchantment|Instant|Sorcery|Planeswalker|[A-Z][a-z]+) )?spells? (you cast|your opponents cast|that target ~)? ?cost ((?:\{[^}]+\})+) (less|more) to cast\.$/i))) {
    const colorless = !!m[1];
    ...
    const filter: FilterSpec | undefined =
      !kind ? undefined
      : kind === 'noncreature' ? { noncreature: true }
      : kind === 'instant and sorcery' ? { typeAnyOf: ['Instant', 'Sorcery'] }
      : TYPE_WORD[kind] ? { what: TYPE_WORD[kind] }
      : kind === 'colorless' ? { colorless: true }
      : { subtype: m[1] };
    const amount = manaValueOfCost(m[3]) * (m[4].toLowerCase() === 'less' ? -1 : 1);
    const who = (m[2] ?? '').toLowerCase();
    (st.costModifiers ??= []).push({ amount, filter: colorless ? { ...(filter ?? {}), colorless: true } : filter, whose: who === 'you cast' ? 'you' : who === 'your opponents cast' ? 'opponent' : 'any' });
    return true;
  }
  if ((m = line.match(/^Spells your opponents cast that target ~ cost ((?:\{[^}]+\})+) more to cast\.$/i))) {
  ...
  if ((m = line.match(/^~ costs ((?:\{[^}]+\})+) less to cast for each (.+)\.$/i))) {
    ...
    const mod: NonNullable<CardDefinition['costModifiers']>[number] = { amount: -manaValueOfCost(m[1]), whose: 'you', self: true };
    if (gy) mod.perGraveyard = info.filter; else mod.per = { ...info.filter, controlledBy: 'you' };
    (st.costModifiers ??= []).push(mod);
    return true;
  }
  if (/^If ~ would die, exile it instead\.$/i.test(line)) { st.flags5.exileInsteadOfDying = true; return true; }
  if (/^If a creature an opponent controls would die, exile it instead\.$/i.test(line)) { st.flags5.exileDyingCreatures = 'opponents'; return true; }
  if (/^If a creature would die, exile it instead\.$/i.test(line)) { st.flags5.exileDyingCreatures = 'all'; return true; }
  if (/^(?:Prevent all damage that would be dealt to ~\.|If damage would be dealt to ~, prevent that damage\.|Prevent all combat damage that would be dealt to ~\.)$/i.test(line)) { st.flags5.preventAllDamageToSelf = true; return true; }
  if ((m = line.match(/^If you would gain life, you gain that much life plus (\d+) instead\.$/i))) { st.flags5.lifeGainModifier = { plus: parseInt(m[1], 10) }; return true; }
  if (/^If you would gain life, you gain twice that much life instead\.$/i.test(line)) { st.flags5.lifeGainModifier = { times: 2 }; return true; }
  if (/^If one or more tokens would be created under your control, twice that many of those tokens are created instead\.$/i.test(line)) { st.flags5.tokenDoubling = true; return true; }
  if ((m = line.match(/^~ can't attack unless defending player controls (?:a|an) ([A-Z][a-z]+)\.$/))) { st.flags5.attackRequiresDefenderSubtype = m[1]; return true; }
  if (/^(?:~ must be blocked if able\.|All creatures able to block ~ do so\.)$/i.test(line)) { st.flags5.mustBeBlocked = true; return true; }
  if (/^Skip your draw step\.$/i.test(line)) { st.flags5.skipDraw = true; return true; }
  if ((m = line.match(/^~ can't be blocked by (artifact|black|white|blue|red|green) creatures\.$/i))) {
    const w = m[1].toLowerCase();
    st.flags5.cantBeBlockedBy = w === 'artifact' ? { types: ['Artifact'] } : { colors: [COLOR_WORDS[w]] };
    return true;
  }
  if ((m = line.match(/^~ can't be blocked by ([A-Z][a-z]+)s\.$/))) { st.flags5.cantBeBlockedBy = { subtypes: [m[1]] }; return true; }
  if (/^Creatures with power less than ~'s power can't block it\.$/i.test(line)) { st.flags5.evasionPowerLessThanSelf = true; return true; }
  if (/^You may choose not to untap ~ during your untap step\.$/i.test(line)) return true; // escolha do jogador: desvirar é o padrão
  if (/^You may cast ~ as though it had flash\.$/i.test(line)) { st.keywords.push('flash'); return true; }
  if (/^You may exert ~ as it attacks\.$/i.test(line)) { st.flags5.canExert = true; return true; }
  if (/^You may have ~ enter (?:the battlefield )?as a copy of (?:any|a) creature on the battlefield(?:, except .+)?\.$/i.test(line)) { st.flags5.copyOnEnter = true; return true; }
  if ((m = line.match(/^Prototype ((?:\{[^}]+\})+) — (\d+)\/(\d+)$/i))) {
    st.flags5.prototype = { cost: m[1], power: parseInt(m[2], 10), toughness: parseInt(m[3], 10) };
    st.castMethods.push({ kind: 'prototype', cost: m[1], label: `protótipo ${m[1]} (${m[2]}/${m[3]})` });
    return true;
  }
  if ((m = line.match(/^Reinforce (\d+)—((?:\{[^}]+\})+)$/i))) {
    st.abilities.push({ kind: 'activated', zone: 'hand', cost: { mana: m[2], discardSelf: true }, targets: [{ what: 'creature' }], effect: [{ op: 'putCounters', what: 'target:0', counter: '+1/+1', count: parseInt(m[1], 10) }], text: `Reforçar ${m[1]} — ${m[2]}` });
    return true;
  }
  if ((m = line.match(/^(?:Raid — )?~ enters with (a|an|\w+) ([\w+/-]+) counters? on it if (.+)\.$/i))) {
    const cond = parseCondG(m[3]);
    const n = num(m[1]) ?? 1;
    if (!cond) return false;
    st.entersWithCounters = { counter: m[2], count: n };
    st.flags5.entersWithCountersIf = cond;
    return true;
  }
  if ((m = line.match(/^~ enters with (?:a|an) ([\w+/-]+) counter on it for each time it was kicked\.$/i))) { st.kickerEnters = { counter: m[1], count: 1 }; return true; }
  if (/^(?:Converge — )?~ enters with (?:a|an) [\w+/-]+ counter on it for each color of mana spent to cast it\.$/i.test(line)) { st.flags3.sunburst = true; return true; }
  ...
  if ((m = line.match(/^(?:Enchanted|Equipped) creature gets ([+-]\d+)\/([+-]\d+) and has ward ((?:\{[^}]+\})+)\.$/i))) {
    st.attachEffect = { ...(st.attachEffect ?? {}), power: parseInt(m[1], 10), toughness: parseInt(m[2], 10), ward: manaValueOfCost(m[3]) };
    return true;
  }
  if (/^Enchanted creature can't attack or block, and its activated abilities can't be activated\.$/i.test(line)) { st.attachEffect = { ...(st.attachEffect ?? {}), cantAttack: true, cantBlock: true }; return true; }
  if (/^You control enchanted permanent\.$/i.test(line)) { st.attachEffect = { ...(st.attachEffect ?? {}), controlHost: true }; return true; }
  ...
  if ((m = line.match(/^~ enters with (?:a|an) ([\w+/-]+) counter on it for each (.+?) in your graveyard\.$/i))) {
    const info = parseNounG(m[2].replace(/ cards?$/i, ' card'));
    if (!info || info.player) return false;
    st.entersWithCounters = { counter: m[1], count: { graveyardCount: 'controller', filter: info.filter } };
    return true;
  }
  ...
  if ((m = line.match(/^Your maximum hand size is (\w+)\.$/i))) { const n = num(m[1]); if (n === null) return false; st.flags13.maxHandSize = n; return true; }
  if (/^If a card or token would be put into your graveyard from anywhere, exile it instead\.$/i.test(line)) { st.flags8.exileInsteadOfGraveyardFor = 'self'; return true; }
  // Exhibition Tidecaller.
  if (/^Whenever you cast an instant or sorcery spell, target player mills three cards\. If five or more mana was spent to cast that spell, that player mills ten cards instead\.$/i.test(line)) {
    st.abilities.push({ kind: 'triggered', trigger: { on: 'youCastSpellOf', filter: { typeAnyOf: ['Instant', 'Sorcery'] } }, targets: [{ what: 'player' }], effect: [{ op: 'if', cond: { kind: 'triggeringManaSpentAtLeast', amount: 5 }, then: [{ op: 'mill', who: 'target:0', count: 10 }], else: [{ op: 'mill', who: 'target:0', count: 3 }] }], text: 'o jogador alvo mói 3 (10 se a mágica custou 5 ou mais)' });
    return true;
  }
  // Grafdigger's Cage.
  if (/^Creature cards in graveyards and libraries can't enter the battlefield\.$/i.test(line)) { st.flags12.cageNoEnterFromGraveyardLibrary = true; return true; }
  if (/^Players can't cast spells from graveyards or libraries\.$/i.test(line)) { st.flags12.cageNoCastFromGraveyardLibrary = true; return true; }
  // Blood Moon / Magus of the Moon.
  if (/^Nonbasic lands are Mountains\.$/i.test(line)) { st.flags12.nonbasicLandsAreMountains = true; return true; }
  // Damping Sphere.
  ...
  if ((m = line.match(/^(?:If (.+?), )?[Yy]ou may (.+?) rather than pay (?:this spell's|~'s) mana cost\.$/)) && !/^You may (?:pay \d+ life and )?exile a (?:white|blue|black|red|green) card from your hand rather than pay/.test(line)) {
    ...
    let alt: CardDefinition['altCost'] | null = null;
    if ((cm = c.match(/^pay (\d+) life$/i))) alt = { payLife: parseInt(cm[1], 10), label: `pague ${cm[1]} de vida` };
    else if ((cm = c.match(/^exile a (white|blue|black|red|green) card from your hand$/i))) alt = { exileFromHand: { count: 1, filter: { color: COLOR_WORDS[cm[1].toLowerCase()] } }, label: `exile uma carta ${COLOR_PT[cm[1].toLowerCase()]} da mão` };
    else if ((cm = c.match(/^exile (two|three) (white|blue|black|red|green) cards from your hand$/i))) alt = { exileFromHand: { count: num(cm[1]) ?? 2, filter: { color: COLOR_WORDS[cm[2].toLowerCase()] } }, label: `exile ${cm[1] === 'two' ? 'duas' : 'três'} cartas ${COLOR_PT[cm[2].toLowerCase()]}s da mão` };
    else if ((cm = c.match(/^pay (\d+) life and exile a (white|blue|black|red|green) card from your hand$/i))) alt = { payLife: parseInt(cm[1], 10), exileFromHand: { count: 1, filter: { color: COLOR_WORDS[cm[2].toLowerCase()] } }, label: `pague ${cm[1]} de vida e exile uma carta ${COLOR_PT[cm[2].toLowerCase()]} da mão` };
    else if ((cm = c.match(/^return (?:a|an) (.+?) you control to its owner's hand$/i))) { const info = parseNounG(cm[1]); if (!info || info.player) return false; alt = { returnLand: { ...info.filter, controlledBy: 'you' }, label: `devolva ${cm[1]} para a mão` }; }
    else if (/^pay \{0\}$/i.test(c)) alt = { free: true, label: 'de graça' };
    if (!alt) return false;
    if (cond) { alt.condition = cond; alt.label += ` (se ${m[1]})`; }
    st.altCost = alt;
    return true;
  }
  if ((m = line.match(/^If (.+?), you may cast (?:it|~|this spell) without paying its mana cost\.$/i))) {
    const cond = parseCondG(m[1]);
    if (!cond) return false;
    st.altCost = { free: true, condition: cond, label: `de graça (se ${m[1]})` };
    return true;
  }
  ...
  if (/^If a card would be put into an opponent's graveyard from anywhere, exile it instead\.$/i.test(line)) { st.flags8.exileInsteadOfGraveyardFor = 'opponents'; return true; }
  if ((m = line.match(/^~ enters with (?:a|an) ([\w+/-]+) counter on it for each instant and sorcery card exiled with it\.$/i))) { st.entersWithCounters = { counter: m[1], count: 'delvedCount' }; return true; }
  if ((m = line.match(/^\{T\}: Add \{C\}\. If you control (?:an? )?(.+?) and (?:an? )?(.+?), add ((?:\{C\})+) instead\.$/i))) {
    const sub = (w: string) => w.trim().split(/\s+/).pop()!;
    const extra = m[3].split('{C}').length - 2;
    st.abilities.push({
      kind: 'activated', cost: { tap: true }, isManaAbility: true, text: `Adicionar {C} (${m[3]} com ${m[1]} e ${m[2]})`,
      effect: [{ op: 'addMana', who: 'controller', mana: ['C'] }, { op: 'if', cond: { kind: 'and', conds: [{ kind: 'controlsAtLeast', count: 1, filter: { what: 'land', subtype: sub(m[1]), controlledBy: 'you' } }, { kind: 'controlsAtLeast', count: 1, filter: { what: 'land', subtype: sub(m[2]), controlledBy: 'you' } }] }, then: [{ op: 'addMana', who: 'controller', mana: Array.from({ length: extra }, () => 'C' as const) }] }],
    });
    return true;
  }
  if ((m = line.match(/^At the beginning of your upkeep, look at the top card of your library\. You may reveal that card\. If (?:a|an) (.+?) card is revealed this way, (.+)\.$/i))) {
    const info = parseNounG(m[1]);
    const parsed = info && !info.player ? parseEffectText(m[2] + '.') : null;
    if (!parsed || specsOf(parsed)) return false;
    st.abilities.push({ kind: 'triggered', trigger: { on: 'upkeep', whose: 'controller' }, effect: [{ op: 'if', cond: { kind: 'topCardIs', filter: info!.filter }, then: parsed.steps }], text: line });
    return true;
  }
  ...
  if (/^You may cast spells from the top of your library\.$/i.test(line)) { st.flags6.castFromLibraryTop = {}; return true; }
  if ((m = line.match(/^You may cast (.+?) spells from the top of your library\.$/i))) {
    const info = parseNounG(m[1]);
    if (!info || info.player) return false;
    st.flags6.castFromLibraryTop = info.filter;
    return true;
  }
  if ((m = line.match(/^If damage would be dealt to ~, prevent (?:that damage|it)\. Remove (?:a|an) ([\w+/-]+) counter from ~\.$/i))) { st.flags6.preventDamageRemoveCounter = m[1]; return true; }
  if (/^~ can't attack alone\.$/i.test(line)) { st.flags6.cantAttackAlone = true; return true; }
  if (/^If ~ would be put into a graveyard from anywhere, reveal ~ and shuffle it into its owner's library instead\.$/i.test(line)) { st.flags6.shuffleInsteadOfGraveyard = true; return true; }
  if (/^Creatures your opponents control enter tapped\.$/i.test(line)) { st.flags6.opponentsCreaturesEnterTapped = true; return true; }
  if (/^Prevent all damage that would be dealt by ~\.$/i.test(line)) { st.flags6.preventsOwnDamage = true; return true; }
  if (/^Each player can't cast more than one spell each turn\.$/i.test(line)) { st.flags6.oneSpellPerTurn = true; return true; }
  if (/^If ~ would be destroyed, regenerate it\.$/i.test(line)) { st.flags6.autoRegenerate = true; return true; }
  if (/^Prevent all damage that would be dealt to enchanted creature\.$/i.test(line)) { st.attachEffect = { ...(st.attachEffect ?? {}), preventsDamage: true }; return true; }
  if ((m = line.match(/^Enchanted creature gets ([+-]\d+)\/([+-]\d+) and is goaded\.$/i))) { st.attachEffect = { ...(st.attachEffect ?? {}), power: parseInt(m[1], 10), toughness: parseInt(m[2], 10), goaded: true }; return true; }
  if (/^Enchanted (?:creature|permanent) can't attack or block, and its activated abilities can't be activated\.$/i.test(line)) { st.attachEffect = { ...(st.attachEffect ?? {}), cantAttack: true, cantBlock: true }; return true; }
  if (/^You may cast ~ as though it had flash\. If you cast it any time a sorcery couldn't have been cast, .+\.$/i.test(line)) {
    st.keywords.push('flash');
    st.softNotes.push('lampejo condicional: se conjurada fora do tempo de feitiço, o controlador sacrifica no fim do turno');
    return true;
  }
  if ((m = line.match(/^When you cycle (?:~|this card), (.+)\.$/i))) {
    const parsed = parseEffectText(m[1] + '.');
    if (!parsed || specsOf(parsed) || parsed.selfExile) return false;
    st.flags6.cyclingTrigger = parsed.steps;
    return true;
  }
  if ((m = line.match(/^During your turn, (.+)\.$/i))) {
    const before = st.abilities.length;
    const innerLine = m[1].replace(/\.$/, '') + '.';
    if (!parseLine(innerLine.charAt(0).toUpperCase() + innerLine.slice(1), st, isSpell, subtypes)) return false;
    const added = st.abilities.slice(before);
    if (added.length === 0 || added.some((a) => a.kind !== 'static')) { st.abilities.length = before; return false; }
    for (const a of added) if (a.kind === 'static') a.condition = { kind: 'yourTurn' };
    return true;
  }
  ...
  if ((m = line.match(/^~ enters tapped unless (.+)\.$/i)) && parseCondG(m[1])) { st.flags6.entersTappedUnlessCond = parseCondG(m[1])!; return true; }
  if ((m = line.match(/^~ enters tapped if (.+)\.$/i)) && parseCondG(m[1])) { st.flags6.entersTappedIf = parseCondG(m[1])!; return true; }
  ...
  if ((m = line.match(/^As an additional cost to cast (?:this spell|~), (.+)\.$/i))) {
    // Leva 5: sacrifício genérico, descarte, vida, exílio do cemitério.
    const c = m[1];
    let cm: RegExpMatchArray | null;
    // Bone Shards: "sacrifice a creature or discard a card" — o conjurador escolhe um dos dois.
    if ((cm = c.match(/^sacrifice (?:a|an) (.+?) or discard a card$/i))) {
      const info = parseNounG(cm[1]);
      if (!info || info.player || info.zone || info.spell || info.any) return false;
      st.additionalCost = { ...(st.additionalCost ?? {}), sacrifice: info.filter, count: 1, discard: 1, either: true };
      return true;
    }
    if ((cm = c.match(/^sacrifice (?:a|an) (.+)$/i))) {
      const info = parseNounG(cm[1]);
      if (!info || info.player || info.zone || info.spell || info.any) return false;
      st.additionalCost = { ...(st.additionalCost ?? {}), sacrifice: info.filter, count: 1 };
      return true;
    }
    if ((cm = c.match(/^sacrifice (\w+) (.+)$/i)) && num(cm[1]) !== null) {
      const info = parseNounG(cm[2]);
      if (!info || info.player || info.zone || info.spell || info.any) return false;
      st.additionalCost = { ...(st.additionalCost ?? {}), sacrifice: info.filter, count: num(cm[1])! };
      return true;
    }
    if ((cm = c.match(/^discard (a|two|three) cards?$/i))) { st.additionalCost = { ...(st.additionalCost ?? {}), discard: num(cm[1]) ?? 1 }; return true; }

    if ((cm = c.match(/^pay (\d+) life$/i))) { st.additionalCost = { ...(st.additionalCost ?? {}), payLife: parseInt(cm[1], 10) }; return true; }
    if ((cm = c.match(/^exile (a|an|\w+) (.+?) cards? from your graveyard$/i))) {
      const n = /^an?$/i.test(cm[1]) ? 1 : num(cm[1]);
      const info = parseNounG(cm[2]);
      if (n === null || !info || info.player || info.zone) return false;
      st.additionalCost = { ...(st.additionalCost ?? {}), exileFromGraveyard: { filter: info.filter, count: n } };
      return true;
    }
    if ((cm = c.match(/^exile (a|an|\w+) cards? from your graveyard$/i))) {
      const n = /^an?$/i.test(cm[1]) ? 1 : num(cm[1]);
      if (n === null) return false;
      st.additionalCost = { ...(st.additionalCost ?? {}), exileFromGraveyard: { filter: {}, count: n } };
      return true;
    }
    return false;
  }
  ...
  if ((m = line.match(/^Flashback—Sacrifice (two|three|four) creatures\.$/i))) { st.flashbackSacrifice = { what: 'creature' }; st.flashbackSacrificeCount = num(m[1]) ?? undefined; return true; }
  if ((m = line.match(/^You may exile (two|three) (white|blue|black|red|green) cards from your hand rather than pay (?:this spell's|~'s) mana cost\.$/i))) {
    st.altCost = { exileFromHand: { count: num(m[1]) ?? 2, filter: { color: COLOR_WORDS[m[2].toLowerCase()] } }, label: `exile ${m[1] === 'two' ? 'duas' : 'três'} cartas ${COLOR_PT[m[2].toLowerCase()]}s da mão` };
    return true;
  }
  ...
  if ((m = line.match(/^Enchant (.+)$/i)) && !/^(?:player|opponent)$/i.test(m[1])) {
    // Leva 5: "Enchant artifact or creature", "Enchant nonland permanent", "Enchant creature you control".
    const info = parseNounG(m[1]);
    const spec = info && !info.player && !info.zone && !info.spell && !info.any ? filterToTargetSpec(info) : null;
    if (!spec) return false;
    const allowed = new Set(['what', 'controlledBy', 'typeAnyOf']);
    if (Object.keys(spec).some((k) => !allowed.has(k))) return false;
    st.enchant = { what: spec.what as NonNullable<CardDefinition['enchant']>['what'], controlledBy: spec.controlledBy, typeAnyOf: spec.typeAnyOf };
    return true;
  }
  ...
  if ((m = line.match(/^((?:When|Whenever|At the beginning of|Landfall — Whenever)[^,]+), (.+)\.$/i))) {
    ...
    if (!header) return false;
    let body = m[2].trim();
    // Bridge from Below: "…, if this card is in your graveyard, …" → a habilidade funciona do cemitério.
    let zone: 'graveyard' | undefined;
    const gyIf = body.match(/^if (?:~|this card) is in your graveyard, (.+)$/i);
    if (gyIf) { zone = 'graveyard'; body = gyIf[1]; }
    if (/^(?:you may )?return (?:~|this card) from your graveyard to the battlefield/i.test(body)) zone = 'graveyard';
    // "If" interveniente: "…, if <condição>, …" → condição checada ao disparar (gramática de condições).
    let condition: Cond | undefined = header.condition;
    const iv = body.match(/^if (.+?), (.+)$/i);
    if (iv) {
      const c = parseCondG(iv[1]);
      if (c) { condition = condition ? { kind: 'and', conds: [condition, c] } : c; body = iv[2]; }
    }
    ...
    const parsed = parseEffectText(body + '.', { pronoun, pronounPlayer });
    if (!parsed || parsed.selfExile) return false;
    const ability: AbilityDef = {
      ...
      effect: parsed.steps,
      text: m[2].trim(),
      oncePerTurn: header.oncePerTurn || undefined,
      zone,
    };
  ...
  // ---- static anthems / lords (gramática composicional: "<substantivo> get +N/+N (and have KW)", "~ gets +1/+1 for each X")
  {
    const sg = parseStaticG(line);
    if (sg && (sg.power !== undefined || sg.toughness !== undefined || sg.keywords || sg.powerPer || sg.toughnessPer)) {
      st.abilities.push({ kind: 'static', filter: sg.filter, selfOnly: sg.selfOnly, hostOnly: sg.hostOnly, power: sg.power, toughness: sg.toughness, keywords: sg.keywords, powerPer: sg.powerPer, toughnessPer: sg.toughnessPer, text: line.replace(/\.$/, '') });
      return true;
    }
  }
  ...
  if ((m = line.match(/^([^:"]+): (.+)$/)) && !/^(?:When|Whenever|At |If |As )/i.test(line)) {
    ...
    let maxPerTurn: number | undefined;
    let actCond: Cond | undefined;
    // Restrições de ativação no fim do texto (podem combinar).
    let rest = true;
    while (rest) {
      rest = false;
    ...
    // Leva 5: habilidades do cemitério ("Return this card from your graveyard to your hand / the battlefield tapped").
    let zone: 'graveyard' | 'hand' | undefined = cost.discardSelf || cost.exileSelfFromHand ? 'hand' : undefined;
    let parsed: ReturnType<typeof parseEffectText>;
    let gm: RegExpMatchArray | null;
    if ((gm = body.match(/^Return (?:~|this card) from your graveyard to (your hand|the battlefield tapped|the battlefield)\.$/i))) {
      zone = 'graveyard';
      const dest = gm[1].toLowerCase();
      parsed = { steps: dest === 'your hand' ? [{ op: 'returnToHand', what: 'self' }] : [{ op: 'returnToBattlefield', what: 'self', tapped: dest.endsWith('tapped') || undefined }] };
    } else parsed = parseEffectText(body);
    if (!parsed || parsed.selfExile) return false;
    ...
    st.abilities.push({
      ...
      maxPerTurn,
      condition: actCond ? { cond: actCond } : undefined,
      isManaAbility: manaOnly && !specsOf(parsed) ? true : undefined,
    });
    return true;
  }
...
export function compileOracleCard(input: OracleInput, diag?: OracleDiagnostics): CardDefinition | null {
  ...
  let text = (input.oracleText ?? '')
    ...
    .replace(/\bThis (creature|land|artifact|enchantment|permanent|Aura|Equipment|Vehicle|Saga|Class|Spacecraft|spell|Siege|battle|token)\b/gi, '~')
    .replace(/[ \t]+/g, ' ')
    .replace(/ \./g, '.')
    .replace(/ ,/g, ',')
    .replace(/ and\/or /g, ' or ')
    .replace(/, reveal it, then shuffle and put that card on top\./g, ', put it on top of your library, then shuffle.')
    .trim();
  ...
  const def: CardDefinition = {
    ...
    flashback: st.flashbackCost ? { cost: st.flashbackCost, payLife: st.flashbackPayLife }
      : st.flashbackSacrifice
        ? { sacrifice: st.flashbackSacrifice, sacrificeCount: st.flashbackSacrificeCount }
        : st.flashbackCost && st.flashbackPayLife
          ? { cost: st.flashbackCost, payLife: st.flashbackPayLife }
        : undefined,
    altCost: st.altCost,
